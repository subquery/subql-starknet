...
function validateAbi(datasources: StarknetRuntimeDatasource[], projectPath: string) {
  ...
  for (const datasource of datasources) {
    ...
    for (const mappingHandler of datasource.mapping.handlers) {
      ...
      if (mappingHandler.kind === StarknetHandlerKind.Event) {
      ...
      if (mappingHandler.kind === StarknetHandlerKind.Call) {
...
//   const inputToSnake = abiObject.map((obj) => obj.type.replace(/\[\]/g, '_arr').toLowerCase()).join('_');
...
export function prepareSortedAssets(
  ...
  for (const d of datasources) {
    ...
    if (!isRuntimeDs(d) && !isCustomDs(d)) continue;
